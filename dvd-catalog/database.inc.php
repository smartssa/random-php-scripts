...
function db_connect($dbname=DBNAME) {
    //global $debug;
    //$debug->Append("Connecting to Database " . DBSERV . "/" . DBUSER . "/" . DBPASS . "/" . DBNAME);
    // reusable database connection for non-persistant usage
    $db = @mysql_connect(DBSERV, DBUSER, DBPASS);
        //or $debug->Append("Cannot connect to the database; So we can't do anything.");
...
function db_query($query) {
    ...
    // debugger.
    //global $debug;
    //$debug->Append("Executing Query: " . $query);
    // execute a query, set some variables, and return the result.
        $db = db_connect();
        $result =  @mysql_query($query,$db);
//        or $debug->Append("Failed to exec Query: " . $query . mysql_error());
...
function db_recordpager ($SQL,$maxrows,$maxpages=0) {
    ...
    // debugger.
//    global $debug;
//    $debug->Append("Recordpaging Rows: " . $maxrows . " Pages: " . $maxpages);
    if ($QUERY_STRING) {
